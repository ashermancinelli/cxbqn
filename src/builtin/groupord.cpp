...
O<Value> GroupOrd::call(u8 nargs, std::vector<O<Value>> args) {
  ...
  for (int i = 1; i < w->N(); i++) {
    tmp[i] = tmp[i - 1] + static_cast<uz>(dynamic_pointer_cast<Number>(w->values[i - 1])->v);
    CXBQN_DEBUG("tmp[{}]={} w[i]={}", i, tmp[i],
                CXBQN_STR_NC(w->values[i - 1]));
  }

  const auto retlen = tmp.back() + static_cast<uz>(dynamic_pointer_cast<Number>(w->values.back())->v);
  CXBQN_DEBUG("return len={}", retlen);

  std::vector<f64> retv(retlen, 0);
  for (int i = 0; i < x->N(); i++) {
    const auto e = dynamic_pointer_cast<Number>(x->values[i])->v;
    if (fge_helper(e, 0.0)) {
